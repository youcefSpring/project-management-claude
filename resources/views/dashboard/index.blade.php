    ...
    return `
        <div class="card border-start border-${color} border-3 mb-2">
            <div class="card-body p-2">
                ...
                ${task.due_date ? `<br><small class="text-muted">{{ __('Due') }}: ${formatDate(task.due_date)}</small>` : ''}
            </div>
        </div>
    `;
}
...
function stopTimer() {
    ...
    if (startTime) {
        ...
        showSuccess(`{{ __("Timer stopped. Elapsed time") }}: ${hours}h ${minutes}m`);
