...
@section('title', __('app.timesheet.log_time_entry'))
@section('page-title', __('app.timesheet.log_new_entry'))
...
<div class="row">
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">{{ __('app.timesheet.entry_information') }}</h5>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('timesheet.store') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('task_id') is-invalid @enderror" id="task_id" name="task_id" required>
                            <option value="">{{ __('app.timesheet.select_task') }}</option>
                            @foreach($availableTasks as $task)
                                <option value="{{ $task->id }}"
                                    {{ old('task_id', request('task_id')) == $task->id ? 'selected' : '' }}>
                                    {{ $task->title }} ({{ optional($task->project)->title ?? __('app.projects.no_project') }})
                                </option>
                            @endforeach
                        </select>
                        @error('task_id')
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="start_time" class="form-label">{{ __('app.timesheet.start_time') }} <span class="text-danger">*</span></label>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="end_time" class="form-label">{{ __('app.timesheet.end_time') }} <span class="text-danger">*</span></label>
                    ...
                    <div class="mb-3">
                        <label for="hours" class="form-label">{{ __('app.timesheet.duration_hours') }}</label>
                        <input type="number" class="form-control @error('hours') is-invalid @enderror"
                               id="hours" name="hours" step="0.25" min="0.1" max="24"
                               value="{{ old('hours') }}" placeholder="{{ __('app.timesheet.enter_hours_placeholder') }}">
                        <small class="form-text text-muted">{{ __('app.timesheet.hours_helper_text') }}</small>
                        @error('hours')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror"
                                  ...
                                  placeholder="{{ __('app.timesheet.describe_work_placeholder') }}">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <button type="submit" class="btn btn-primary">
                            ...
                            {{ __('app.timesheet.log_time_entry') }}
                        </button>
                    </div>
                </form>
            </div>
    ...
    <div class="col-md-4">
        ...
        <div class="card mt-4">
            <div class="card-header">
                <h5 class="mb-0">{{ __('app.timesheet.guidelines') }}</h5>
            </div>
            <div class="card-body">
                <h6>{{ __('app.timesheet.time_tracking_tips') }}</h6>
                <ul class="small text-muted">
                    <li>{{ __('app.timesheet.tip_be_accurate') }}</li>
                    <li>{{ __('app.timesheet.tip_detailed_descriptions') }}</li>
                    <li>{{ __('app.timesheet.tip_log_daily') }}</li>
                    <li>{{ __('app.timesheet.tip_max_hours') }}</li>
                </ul>
            </div>
        </div>
    </div>
